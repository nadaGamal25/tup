...
export default function JtEdit() {
  ...
  async function sendPricesToApi() {
    ...
    try {
      const {data} = await axios.post(`https://dashboard.go-tex.net/api/jt/edit`,Prices,
      {
        headers: {
...
function submitPricesForm(e){
  ...
  async function getCompaniesDetailsOrders() {
    try {
      ...
      setPrices({
        ...Prices,
        status:companiesPrices[6].status, 
        userprice:companiesPrices[6].userprice,
        marketerprice:companiesPrices[6].marketerprice,
        kgprice:companiesPrices[6].kgprice,
        userCodPrice:companiesPrices[6].codprice,
        maxcodmarkteer:companiesPrices[6].maxcodmarkteer,
        mincodmarkteer: companiesPrices[6].mincodmarkteer, 
      });
    } catch (error) {
      console.error(error);
    }
          ...
          <div className=" py-3">
            <div className="edit-form">
              <div className="p-saee p-3">
                <h5 className="text-center mb-3">أسعار شركة J&T </h5>
