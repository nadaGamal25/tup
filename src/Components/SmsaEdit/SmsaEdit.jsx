...
export default function SmsaEdit() {
  ...
  async function sendPricesToApi() {
    ...
    try {
      const {data} = await axios.post(`https://dashboard.go-tex.net/api/smsa/edit`, Prices,
      {
        headers: {
...
function submitPricesForm(e){
  ...
  async function getCompaniesDetailsOrders() {
    try {
      ...
      setPrices({
        ...Prices,
        status:companiesPrices[2].status, 
        userprice:companiesPrices[2].userprice,
        marketerprice:companiesPrices[2].marketerprice,
        kgprice:companiesPrices[2].kgprice,
        userCodPrice:companiesPrices[2].codprice,
        maxcodmarkteer:companiesPrices[2].maxcodmarkteer,
        mincodmarkteer: companiesPrices[2].mincodmarkteer, 
      });
    } catch (error) {
      console.error(error);
    }
